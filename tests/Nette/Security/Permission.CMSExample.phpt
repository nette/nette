...
$user = new Identity(1, array('user'));
$staff = new Identity(2, array('staff'));
$editor = new Identity(3, array('editor'));
$administrator = new Identity(4, array('administrator'));
$marketing = new Identity(5, array('marketing'));

$acl = new Permission;
$acl->addRole('user');
$acl->addRole('staff', 'user');  // staff inherits permissions from user
...
// Staff inherits view privilege from user, but also needs additional privileges
...
Assert::true( $acl->isAllowed($user, NULL, 'view') );
Assert::false( $acl->isAllowed($user, NULL, 'edit') );
Assert::false( $acl->isAllowed($user, NULL, 'submit') );
Assert::false( $acl->isAllowed($user, NULL, 'revise') );
Assert::false( $acl->isAllowed($user, NULL, 'publish') );
Assert::false( $acl->isAllowed($user, NULL, 'archive') );
Assert::false( $acl->isAllowed($user, NULL, 'delete') );
Assert::false( $acl->isAllowed($user, NULL, 'unknown') );
Assert::false( $acl->isAllowed($user) );
...
Assert::true( $acl->isAllowed($user, 'pending', 'view') );
