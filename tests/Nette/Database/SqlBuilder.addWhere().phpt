...
try {
	...
	Assert::same('Nette\InvalidArgumentException', get_class($e));
	Assert::same('Selection argument must have defined a select column.', $e->getMessage());

	$e = $e->getPrevious();
	Assert::same('LogicException', get_class($e));
	Assert::same('Table "book_tag" does not have a primary key.', $e->getMessage());
}
