	...
	private function setupDatabase(ContainerBuilder $container, array $config)
	{
		...
		foreach ((array) $config as $name => $info) {
			...
			$connection = $container->addDefinition($this->prefix("database.$name"))
				...
				->addSetup('setContainer', array(
					new Nette\DI\Statement('Nette\Database\Container', array('@self', $reflection)),
				))
			...
			$container->addDefinition($this->prefix("database.$name.container"))
				->setClass('Nette\Database\Container')
				->setFactory(array($connection, 'getContainer'))
				->setAutowired($info['autowired']);
